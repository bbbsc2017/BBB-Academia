...
import { updateFolder, updateFolderThumbnail } from '@services/folders/folders'
...
function EditFolderModal({ folder, closeModal, onChanged }: Props) {
  ...
  const existingThumb = folder?.thumbnail_image
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      const toastId = toast.loading(t('library.saving_folder'))
      try {
        await updateFolder(
          folder.folder_uuid,
          {
        ...
        if (file) {
          try {
            await updateFolderThumbnail(folder.folder_uuid, file, access_token)
          } catch (e: any) {
            toast.error(e?.message || t('library.thumbnail_error'))
          }
        }
        toast.success(t('library.folder_updated_success'))
        closeModal()
        onChanged?.()
      } catch (error: any) {
        toast.error(error?.message || t('library.folder_updated_error'))
      } finally {
  ...
  return (
    <FormLayout onSubmit={formik.handleSubmit}>
      <FormField name="name">
        <FormLabelAndMessage label={t('library.folder_name')} message={formik.errors.name as string} />
      ...
      <FormField name="description">
        <FormLabelAndMessage label={t('library.description')} message={formik.errors.description as string} />
        <Form.Control asChild>
          ...
          <button
            ...
            className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-all ${
              formik.values.public
                ? 'bg-white border-[#00a9bf]/25 ring-1 ring-[#00a9bf]/10 text-gray-900'
      ...
      <FolderAppearance color={color} onColorChange={setColor} file={file} onFileChange={setFile} existingThumbUrl={existingThumb} />
