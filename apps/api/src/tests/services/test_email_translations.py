...
class TestT:
    def test_translates_to_target_language(self):
        ...
        assert t("es", "invitation.heading") == "¡Te han invitado!"
    ...
    def test_falls_back_to_english_when_key_missing_in_locale(self):
        # org_created isn't translated for es — we expect the English bundle's
        # value back rather than a raw key.
        assert t("es", "org_created.heading", org_name="Acme") == EMAIL_TRANSLATIONS["en"]["org_created.heading"].format(org_name="Acme")

    def test_returns_key_itself_when_key_unknown_everywhere(self):
        # No locale has this key — t() must not raise.
        assert t("es", "totally.made.up.key") == "totally.made.up.key"

    def test_format_kwargs_are_interpolated(self):
        rendered = t("es", "invitation.subject", org_name="Acme")
    ...
    def test_missing_format_kwargs_return_template_safely(self):
        ...
        rendered = t("es", "invitation.subject")
